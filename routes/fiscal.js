const express = require('express');
const router = express.Router();
const { query, withTransaction } = require('../db/database');

// Returns all fiscal years that have been closed
router.get('/years', async (req, res) => {
  try {
    const { rows }        = await query("SELECT DISTINCT substr(date, 1, 4) as year FROM journal_entries ORDER BY year DESC");
    const { rows: closed} = await query("SELECT DISTINCT substr(date, 1, 4) as year FROM journal_entries WHERE entry_type = 'closing' ORDER BY year DESC");
    res.json({ years: rows.map(r => r.year), closedYears: closed.map(r => r.year) });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// Preview what the year-end closing entry will look like
router.get('/preview-close', async (req, res) => {
  try {
    const year = req.query.year || new Date().getFullYear();
    const from = `${year}-01-01`;
    const to   = `${year}-12-31`;

    const { rows } = await query(`
      SELECT a.id, a.code, a.name, a.type, a.normal_balance,
             COALESCE(SUM(COALESCE(jl.base_debit, jl.debit)), 0)   AS total_debits,
             COALESCE(SUM(COALESCE(jl.base_credit, jl.credit)), 0) AS total_credits
      FROM accounts a
      LEFT JOIN journal_lines jl ON a.id = jl.account_id
      LEFT JOIN journal_entries je ON jl.entry_id = je.id
        AND je.status = 'posted'
        AND je.entry_type != 'closing'
        AND je.date >= $1 AND je.date <= $2
      WHERE a.type IN ('Revenue', 'COGS', 'Expense')
      GROUP BY a.id, a.code, a.name, a.type, a.normal_balance
      ORDER BY a.code
    `, [from, to]);

    const lines = [];
    let netIncome = 0;

    for (const row of rows) {
      const balance = row.normal_balance === 'Debit'
        ? (parseFloat(row.total_debits) || 0) - (parseFloat(row.total_credits) || 0)
        : (parseFloat(row.total_credits) || 0) - (parseFloat(row.total_debits) || 0);
      if (Math.abs(balance) < 0.005) continue;

      if (row.type === 'Revenue') {
        lines.push({ account_id: row.id, code: row.code, name: row.name, type: row.type, debit: balance, credit: 0 });
        netIncome += balance;
      } else {
        lines.push({ account_id: row.id, code: row.code, name: row.name, type: row.type, debit: 0, credit: balance });
        netIncome -= balance;
      }
    }

    const { rows: [reAccount] } = await query("SELECT id, code, name FROM accounts WHERE code = '3200'");
    if (reAccount) {
      if (netIncome > 0)       lines.push({ account_id: reAccount.id, code: reAccount.code, name: reAccount.name, type: 'Equity', debit: 0, credit: netIncome });
      else if (netIncome < 0)  lines.push({ account_id: reAccount.id, code: reAccount.code, name: reAccount.name, type: 'Equity', debit: Math.abs(netIncome), credit: 0 });
    }

    res.json({ year, lines, netIncome });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// Execute year-end closing
router.post('/close-year', async (req, res) => {
  const { year } = req.body;
  if (!year) return res.status(400).json({ error: 'year is required' });

  try {
    const { rows: [already] } = await query(
      "SELECT id FROM journal_entries WHERE entry_type = 'closing' AND date LIKE $1",
      [`${year}%`]
    );
    if (already) return res.status(400).json({ error: `Year ${year} has already been closed.` });

    const from = `${year}-01-01`;
    const to   = `${year}-12-31`;

    const { rows } = await query(`
      SELECT a.id, a.code, a.name, a.type, a.normal_balance,
             COALESCE(SUM(COALESCE(jl.base_debit, jl.debit)), 0)   AS total_debits,
             COALESCE(SUM(COALESCE(jl.base_credit, jl.credit)), 0) AS total_credits
      FROM accounts a
      LEFT JOIN journal_lines jl ON a.id = jl.account_id
      LEFT JOIN journal_entries je ON jl.entry_id = je.id
        AND je.status = 'posted'
        AND je.entry_type != 'closing'
        AND je.date >= $1 AND je.date <= $2
      WHERE a.type IN ('Revenue', 'COGS', 'Expense')
      GROUP BY a.id, a.code, a.name, a.type, a.normal_balance
      ORDER BY a.code
    `, [from, to]);

    const closingLines = [];
    let netIncome = 0;

    for (const row of rows) {
      const balance = row.normal_balance === 'Debit'
        ? (parseFloat(row.total_debits) || 0) - (parseFloat(row.total_credits) || 0)
        : (parseFloat(row.total_credits) || 0) - (parseFloat(row.total_debits) || 0);
      if (Math.abs(balance) < 0.005) continue;

      if (row.type === 'Revenue') { closingLines.push({ account_id: row.id, debit: balance, credit: 0 }); netIncome += balance; }
      else                        { closingLines.push({ account_id: row.id, debit: 0, credit: balance }); netIncome -= balance; }
    }

    if (closingLines.length === 0) {
      return res.status(400).json({ error: 'No revenue or expense accounts have balances for this year. Nothing to close.' });
    }

    const { rows: [reAccount] } = await query("SELECT id FROM accounts WHERE code = '3200'");
    if (!reAccount) return res.status(500).json({ error: 'Retained Earnings account (3200) not found.' });

    if (Math.abs(netIncome) >= 0.005) {
      if (netIncome > 0) closingLines.push({ account_id: reAccount.id, debit: 0, credit: netIncome });
      else               closingLines.push({ account_id: reAccount.id, debit: Math.abs(netIncome), credit: 0 });
    }

    const entry = await withTransaction(async (client) => {
      const { rows: [last] } = await client.query(
        "SELECT reference FROM journal_entries WHERE reference LIKE 'CE-%' ORDER BY id DESC LIMIT 1"
      );
      const num       = last ? parseInt(last.reference.replace('CE-', ''), 10) + 1 : 1;
      const reference = `CE-${String(num).padStart(4, '0')}`;

      const { rows: [inserted] } = await client.query(`
        INSERT INTO journal_entries (date, reference, description, status, entry_type)
        VALUES ($1, $2, $3, 'posted', 'closing') RETURNING *
      `, [`${year}-12-31`, reference, `Year-End Closing Entry — FY ${year}`]);

      for (const line of closingLines) {
        await client.query(
          'INSERT INTO journal_lines (entry_id, account_id, debit, credit, base_debit, base_credit) VALUES ($1, $2, $3, $4, $5, $6)',
          [inserted.id, line.account_id, line.debit, line.credit, line.debit, line.credit]
        );
      }
      return inserted;
    });

    res.json({ success: true, entry, netIncome });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// Get opening balances for a year
router.get('/opening-balances', async (req, res) => {
  try {
    const year      = req.query.year || new Date().getFullYear();
    const priorDec31 = `${parseInt(year) - 1}-12-31`;

    const { rows } = await query(`
      SELECT a.id, a.code, a.name, a.type, a.normal_balance,
             COALESCE(SUM(COALESCE(jl.base_debit, jl.debit)), 0)   AS total_debits,
             COALESCE(SUM(COALESCE(jl.base_credit, jl.credit)), 0) AS total_credits
      FROM accounts a
      LEFT JOIN journal_lines jl ON a.id = jl.account_id
      LEFT JOIN journal_entries je ON jl.entry_id = je.id
        AND je.status = 'posted'
        AND je.date <= $1
      WHERE a.type IN ('Asset', 'Liability', 'Equity')
      GROUP BY a.id, a.code, a.name, a.type, a.normal_balance
      ORDER BY a.code
    `, [priorDec31]);

    const balances = rows.map(row => ({
      ...row,
      total_debits:  parseFloat(row.total_debits)  || 0,
      total_credits: parseFloat(row.total_credits) || 0,
      balance: row.normal_balance === 'Debit'
        ? (parseFloat(row.total_debits) || 0) - (parseFloat(row.total_credits) || 0)
        : (parseFloat(row.total_credits) || 0) - (parseFloat(row.total_debits) || 0),
    })).filter(r => Math.abs(r.balance) >= 0.005);

    res.json({ year, priorDate: priorDec31, balances });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

module.exports = router;

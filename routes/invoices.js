...
const MAR   = 50;              // page margin (pts)
const PW    = 612;             // letter width
const RIGHT = PW - MAR;       // right-side x
...
router.get('/receivable/:id', (req, res) => {
  ...
  const biz = db.prepare('SELECT * FROM business_settings LIMIT 1').get() || {};
  ...
  const doc = new PDFDocument({ size: 'LETTER', margin: MAR, info: {
    ...
    Author:  biz.business_name || 'My Business',
  ...
  doc.fontSize(22).font('Helvetica-Bold').fillColor(C.dark)
     .text(biz.business_name || 'My Business', MAR, 50);
  ...
  if (biz.tax_id)              { doc.text(`TIN: ${biz.tax_id}`, MAR, by); by += 13; }
  if (biz.registration_number) { doc.text(`Reg. No.: ${biz.registration_number}`, MAR, by); }
  ...
  const details = [
    ['Invoice #',  invNum],
    ['Date',       fmtDate(rec.created_at?.split('T')[0])],
    ['Due Date',   fmtDate(rec.due_date)],
  ];
  if (rec.currency && rec.currency !== (biz.currency || 'USD')) {
  ...
  // Item rows
  const items = [
    { desc: rec.description || 'Services rendered', amount: rec.amount },
  ];
  ...
  if (rec.paid_amount > 0) {
    addRow('Amount Paid', `− ${fmt(rec.paid_amount)}`, false, C.success);
  }

  // Separator above balance due
  ...
  doc.rect(MAR, totY, 4, 26).fill(statusFg);           // left accent bar
  ...
  doc.fontSize(9).font('Helvetica-Bold').fillColor(C.dark)
     .text('Thank you for your business!', MAR, footY + 12, {
       width: RIGHT - MAR, align: 'center',
     });
  doc.fontSize(8).font('Helvetica').fillColor(C.gray)
     .text(
       biz.address
         ? `${biz.business_name || ''} · ${biz.address.split('\n')[0].trim()}`
         : (biz.business_name || ''),
       MAR, footY + 27,
       { width: RIGHT - MAR, align: 'center' },
     );
